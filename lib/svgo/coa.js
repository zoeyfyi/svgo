    ...
    .act(function(opts, args) {
        ...
        if (typeof process == 'object' && process.versions && process.versions.node && PKG && PKG.engines.node) {
            ...
            if (parseFloat(process.versions.node) < parseFloat(nodeVersion)) {
                console.error(`Error: ${PKG.name} requires Node.js version ${nodeVersion} or higher.`);
                process.exit(1);
            }
        }
...
function optimizeFromString(svgstr, config, datauri, input, output) {
    ...
    svgo.optimize(svgstr, {input: 'string'}).then(function(result) {
        if (datauri) {
...
function showAvailablePlugins() {
    ...
    list = list.concat.apply(list, svgo.config.plugins).sort((a, b) => a.name.localeCompare(b.name));
